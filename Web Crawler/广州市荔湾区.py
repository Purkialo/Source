...
def get_link_content_text(url):
    ...
    context = sub_sp.find('div',attrs={'class':'content_content'})
    #去除style标签
    try:
    ...
    author_name = sub_sp.find(text=re.compile(u"来源"))
    #if(author_name == None):
    #    author_name = sub_sp.find(text=re.compile(u"发布机构"))
    #print(author_name)
    try:
        author_name = author_name.split('：')[2]
...
def get_content(bs,save_context):
    ...
    #找出所有新闻链接和对应时间
    link_a_all = bs.find_all('span',attrs={'class':'toutiao'})
    time_all = bs.find_all('span',attrs={'style':'font-size: 12px; color: #666666'})
    #print(link_a_all)
    #print(time_all)
...
#进入主页
driver.get("http://www.lw.gov.cn/sofpro/search/search.jsp?keyword=%E7%A4%BE%E5%8C%BA")
