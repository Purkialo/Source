...
train_x, train_y = generate_data( np.cos( np.linspace(0, test_start, training_examples) ) )
test_x, test_y = generate_data( np.cos( np.linspace(test_start, test_end, testing_examples) ) )
...
with session.as_default() as sess:
    ...
    # plt.plot(test_y, 'g--', label='real cos')
    # plt.legend()
    # plt.show()